...
/// Define: Console_STRING_T
/// Define/typedef this to your preference to override Console's string type.
///
/// Defaults to std::string with C++ and char* with C.
#if 0
#define Console_STRING_T char*
#endif

#ifndef Console_INLINE
#ifdef _MSC_VER
#define Console_INLINE __inline
#else
#define Console_INLINE static __inline__
#endif
#endif
...
namespace Console {
	Console_INLINE void locate(int x, int y);
}
#else
#include <stdio.h> // for getch() / printf()
#include <string.h> // for strlen()
Console_INLINE void locate(int x, int y); // Forward declare for C to avoid warnings
#endif // __cplusplus
...
#ifndef gotoxy
/// Function: gotoxy
/// Same as <Console.locate>.
Console_INLINE void gotoxy(int x, int y) {
#ifdef __cplusplus
	Console::
#endif
		locate(x, y);
...
/// under namespace Console. That is because some platforms have them defined
/// outside of Console.
namespace Console {
#endif
	...
	* Console_STRING_T - String type depending on which one of C or C++ is used
	* Console_PRINT(str) - Printing macro independent of C/C++
	*/

#ifdef __cplusplus
#ifndef Console_STRING_T
	typedef std::string Console_STRING_T;
#endif // Console_STRING_T

#define Console_PRINT(st) do { std::cout << st; } while(false)
#else // __cplusplus
#ifndef Console_STRING_T
	typedef const char* Console_STRING_T;
#endif // Console_STRING_T

#define Console_PRINT(st) printf("%s", st)
#endif // __cplusplus
	...
	*/
	const Console_STRING_T ANSI_CLS = "\033[2J\033[3J";
	const Console_STRING_T ANSI_CONSOLE_TITLE_PRE = "\033]0;";
	const Console_STRING_T ANSI_CONSOLE_TITLE_POST = "\007";
	const Console_STRING_T ANSI_ATTRIBUTE_RESET = "\033[0m";
	const Console_STRING_T ANSI_CURSOR_HIDE = "\033[?25l";
	const Console_STRING_T ANSI_CURSOR_SHOW = "\033[?25h";
	const Console_STRING_T ANSI_CURSOR_HOME = "\033[H";
	const Console_STRING_T ANSI_BLACK = "\033[22;30m";
	const Console_STRING_T ANSI_RED = "\033[22;31m";
	const Console_STRING_T ANSI_GREEN = "\033[22;32m";
	const Console_STRING_T ANSI_BROWN = "\033[22;33m";
	const Console_STRING_T ANSI_BLUE = "\033[22;34m";
	const Console_STRING_T ANSI_MAGENTA = "\033[22;35m";
	const Console_STRING_T ANSI_CYAN = "\033[22;36m";
	const Console_STRING_T ANSI_GREY = "\033[22;37m";
	const Console_STRING_T ANSI_DARKGREY = "\033[01;30m";
	const Console_STRING_T ANSI_LIGHTRED = "\033[01;31m";
	const Console_STRING_T ANSI_LIGHTGREEN = "\033[01;32m";
	const Console_STRING_T ANSI_YELLOW = "\033[01;33m";
	const Console_STRING_T ANSI_LIGHTBLUE = "\033[01;34m";
	const Console_STRING_T ANSI_LIGHTMAGENTA = "\033[01;35m";
	const Console_STRING_T ANSI_LIGHTCYAN = "\033[01;36m";
	const Console_STRING_T ANSI_WHITE = "\033[01;37m";
	const Console_STRING_T ANSI_BACKGROUND_BLACK = "\033[40m";
	const Console_STRING_T ANSI_BACKGROUND_RED = "\033[41m";
	const Console_STRING_T ANSI_BACKGROUND_GREEN = "\033[42m";
	const Console_STRING_T ANSI_BACKGROUND_YELLOW = "\033[43m";
	const Console_STRING_T ANSI_BACKGROUND_BLUE = "\033[44m";
	const Console_STRING_T ANSI_BACKGROUND_MAGENTA = "\033[45m";
	const Console_STRING_T ANSI_BACKGROUND_CYAN = "\033[46m";
	const Console_STRING_T ANSI_BACKGROUND_WHITE = "\033[47m";
	...
	*/
	...
	Console_INLINE int getkey(void) {
#ifndef _WIN32
	...
	Console_INLINE int nb_getch(void) {
	...
	///
	/// See <Color Codes>
	Console_INLINE Console_STRING_T getANSIColor(const int c) {
		switch (c) {
	...
	///
	/// See <Color Codes>
	Console_INLINE Console_STRING_T getANSIBackgroundColor(const int c) {
		switch (c) {
	...
	///
	/// See <Color Codes>
	Console_INLINE void setColor(int c) {
#if defined(_WIN32) && !defined(Console_USE_ANSI)
	...
	///
	/// See <Color Codes>
	Console_INLINE void setBackgroundColor(int c) {
#if defined(_WIN32) && !defined(Console_USE_ANSI)
		HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
		CONSOLE_SCREEN_BUFFER_INFO csbi;
...
#else
		Console_PRINT(getANSIBackgroundColor(c));
#endif
	}
	...
	/// See <Color Codes>
	/// See <resetColor>
	Console_INLINE int saveDefaultColor(void) {
#if defined(_WIN32) && !defined(Console_USE_ANSI)
...
#else
		Console_PRINT(ANSI_CLS);
		Console_PRINT(ANSI_CURSOR_HOME);
#endif
	}
...
#ifdef __cplusplus
	Console_INLINE void setString(const Console_STRING_T & str_) {
		const char * const str = str_.data();
		unsigned int len = str_.size();
#else // __cplusplus
	Console_INLINE void setString(Console_STRING_T str) {
	...
	Console_INLINE void setChar(char ch) {
		const char buf[] = { ch, 0 };
		setString(buf);
	}
	...
	Console_INLINE void setCursorVisibility(char visible) {
#if defined(_WIN32) && !defined(Console_USE_ANSI)
		HANDLE hConsoleOutput = GetStdHandle(STD_OUTPUT_HANDLE);
...
#else // _WIN32 || USE_ANSI
		Console_PRINT((visible ? ANSI_CURSOR_SHOW : ANSI_CURSOR_HIDE));
#endif // _WIN32 || USE_ANSI
	}

	/// Function: hidecursor
	/// Hides the cursor.
	Console_INLINE void hidecursor(void) {
		setCursorVisibility(0);
	}

	/// Function: showcursor
	/// Shows the cursor.
	Console_INLINE void showcursor(void) {
		setCursorVisibility(1);
	}

	/// Function: msleep
	/// Waits given number of milliseconds before continuing.
	Console_INLINE void msleep(unsigned int ms) {
#ifdef _WIN32
		Sleep(ms);
#else
	...
	template <class T> void anykey(const T& msg) {
		Console_PRINT(msg);
#else
	Console_INLINE void anykey(Console_STRING_T msg) {
		if (msg)
			Console_PRINT(msg);
#endif // __cplusplus
		getch();
	...
	Console_INLINE void setConsoleTitle(Console_STRING_T title) {
...
#else
		Console_PRINT(ANSI_CONSOLE_TITLE_PRE);
		Console_PRINT(true_title);
		Console_PRINT(ANSI_CONSOLE_TITLE_POST);
#endif // defined(_WIN32) && !defined(Console_USE_ANSI)
	}
...
#ifdef __cplusplus
	/// Class: CursorHider
	/// RAII OOP wrapper for <Console.hidecursor>.
